    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(datosuserlogged.isValid() == true)
            {
                ...
                // administrator
                if (datosuserlogged.getIdCargo().equals("1")) {
                    response.sendRedirect("PanelAdministrador.jsp");
                }
                // jefe almacen
                else if (datosuserlogged.getIdCargo().equals("2")) {
                    response.sendRedirect("PanelJefeAlmacen.jsp");
                }
                //vendedeor
                    else if (datosuserlogged.getIdCargo().equals("5")) {
                    response.sendRedirect("PanelVendedor.jsp");
                }
                    else if (datosuserlogged.getIdCargo().equals("3")) {
                    response.sendRedirect("PanelEncargadoSucursal.jsp");
                }
            }
            else
            {
